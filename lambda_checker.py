...
def lambda_checker(lambda_function):
	...
	try:
		...
		function = file('lambda_checker_' + lambda_function + '_' + timestamp + '.zip', 'w')
	...
	if(args.all is True):
		...
		try:
			for functions in response['Functions']:
				print('[+] [' + str(cont_func) + '/' + str(len(response['Functions'])) + '] Results for ' + functions['FunctionName'])
				print('')
			...
			print('[-] [' + str(cont_func) + '/' + str(len(response['Functions'])) + '] ERROR IN FUNCTION ' + functions['FunctionName'] + ' SKIPPING...')
	else:
