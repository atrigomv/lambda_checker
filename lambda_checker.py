	...
	try:
		...
		#	#	Checking if the role is associated to other Lambda functions:
			...
			for function in response['Functions']:
				if((str(function['Role']) == str(data['Configuration']['Role'])) and (function['FunctionName'] != str(lambda_function))):
		...
		function = file('lambda_checker_' + str(lambda_function) + '_' + timestamp + '.zip', 'w')
		function.write(download.read())
		function.close()
		if(zipfile.is_zipfile('lambda_checker_' + str(lambda_function) + '_' + timestamp + '.zip')):
			zip_ref = zipfile.ZipFile(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp + '.zip','r')
			zip_ref.extractall(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp)
			zip_ref.close()
		if(tarfile.is_tarfile('lambda_checker_' + str(lambda_function) + '_' + timestamp + '.zip')):
                       	tar_ref = tarfile.open(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp + '.zip')
                       	tar_ref.extractall(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp)
                       	tar_ref.close()
		os.remove(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp + '.zip')
		for obj in listdir(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp):
			if(str(obj).find('.') != -1):
				if(((str(obj).split('.')[1]).upper() == 'MD') or ((str(obj).split('.')[1]).upper() == 'TYPES') or ((str(obj).split('.')[1]).upper() == 'SO')):
					continue
				else:
				...
				continue
			try:
				f = open(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp + '/' + str(obj))
			except:
				continue
			num_line = 0
			while True:
		...
		#		# Searching for use of assert conditions:
					...
					if fa:
						write_vuln(lambda_function,'VA7', 'Use of assertion clauses', 'LOW - Use of assertion clauses in line ' + str(num_line) + ' of ' + str(obj) +' file: (' + linea.strip() + '). An assertion condition can be bypassed setting the debug mode to false which can execute non-expected situations in the code (for example, authorization issues)','LOW','Medium')
			f.close()

		#	Remove folder if option 'd' is not selected:
		if(args.download is False):
			shutil.rmtree(os.getcwd() + '/lambda_checker_' + str(lambda_function) + '_' + timestamp + '/')
