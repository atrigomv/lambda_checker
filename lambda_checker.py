...
parser.add_argument('-p', '--piidata', help='If is checked the script will not find for personal data (spanish)', action='store_true')
				...
				if(args.piidata is False):
					for name in names:
						if(linea_mayus.find(name.upper()) != -1):
							write_vuln('VA8','Personal data found','LOW - Personal data found (' + name.upper() + ') in line ' + str(num_line) + ' of ' + str(obj) + ' file. (' + linea.strip() + ')','LOW','Medium')
