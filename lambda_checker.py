	...
	try:
		...
		if((data['Configuration']['Role'].find('role/service-role') and data['Configuration']['Role'].find(args.function))!= -1):
			...
			try:
				...
				role_name = str(data['Configuration']['Role']).split('/')[1]
		...
		#	#	Checking if the Policy has as resource *
				...
				if(data_role != ''):
					print('[-] The Lambda has the role ' + role_name + ' but it was impossible to retrieve their permissions. Some checks will be not verified')
				else:
